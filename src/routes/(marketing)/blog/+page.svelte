...
<div class="py-8 lg:py-12 px-6 max-w-lg mx-auto">
  ...
  {#each sortedPosts as post}
    <a href={post.link}>
      <div class="card my-6 bg-neutral-800 shadow-xl flex-row overflow-hidden">
        <div class="flex-none w-6 md:w-32 bg-secondary"></div>
        <div class="py-6 px-6">
          <div class="text-xl text-white">{post.title}</div>
          ...
          <div class="text-gray-50">{post.description}</div>
        </div>
      </div>
    </a>
  {/each}
